...
class Tx_FormBase_Finishers_EmailFinisher extends Tx_FormBase_Core_Model_AbstractFinisher {
	...
	protected function executeInternal() {
		...
		if ($subject === NULL) {
			throw $this->objectManager->create('Tx_FormBase_Exception_FinisherException','The option "subject" must be set for the EmailFinisher.', 1327060320);
		}
		if ($recipientAddress === NULL) {
			throw $this->objectManager->create('Tx_FormBase_Exception_FinisherException','The option "recipientAddress" must be set for the EmailFinisher.', 1327060200);
		}
		if ($senderAddress === NULL) {
			throw $this->objectManager->create('Tx_FormBase_Exception_FinisherException','The option "senderAddress" must be set for the EmailFinisher.', 1327060210);
		}
	...
	/**
	 * @return Tx_FormBase_View_StandaloneView
	 * @throws Tx_FormBase_Exception_FinisherException
	 */
	protected function initializeStandaloneView() {
		$standaloneView = $this->objectManager->create('Tx_FormBase_View_StandaloneView');
		if (!isset($this->options['templatePathAndFilename'])) {
			throw $this->objectManager->create('Tx_FormBase_Exception_FinisherException','The option "templatePathAndFilename" must be set for the EmailFinisher.', 1327058829);
		}
